...
export function generateP2TRCommitNoteInfo(payload: NotePayload, pubkey: Buffer, network: Network) {
  const xOnlyPubkey = toXOnly(pubkey)

  const note_script = script.fromASM(buildCommitNoteScript(payload, xOnlyPubkey))
...
function addPsbtPayUtxos(pubkey: Buffer, psbt: Psbt, utxos: IUtxo[], network: Network) {
  let totalInput = 0
...
export async function createP2TRNotePsbt(
  ...
  const psbtOption: PsbtOption = {
    autoFinalized: false,
    toSignInputs: [],
  }

  for (let i = 0; i < noteUtxos.length; i++) {
    psbtOption.toSignInputs.push({
      index: i,
  ...
  const new_psbt_hex = await n20_wallet.btc_wallet.signPsbt(psbt.toHex(), psbtOption)
  ...
  const txRet = psbt_new.extractTransaction()
  return txRet
}

export async function createP2TRCommitNotePsbt(
  n20_wallet: N20_Wallet,
  notePayload: NotePayload,
  noteUtxo: IUtxo,
  payUtxos: IUtxo[],
  ...
  if (noteUtxo.type !== 'P2TR-COMMIT-NOTE') {
    throw new Error('Invalid noteUtxo type')
  }

  const p2note = generateP2TRCommitNoteInfo(
    notePayload,
    n20_wallet.currentAccount.publicKey,
    network
  )
  ...
  const fastEstimatedSize =
    (psbt.txInputs.length * 51993) / 628 + ((psbt.txOutputs.length + 1) * 327) / 9 + extSize / 4 // TODO: check this formula, the result if little big than real size
...
export async function createCoinPsbt(
  n20_wallet: N20_Wallet,
  utxos: IUtxo[],
  to: ISendToAddress[],
  change: string,
  ...
  const psbt = new Psbt({ network })
  const totalInput = addPsbtPayUtxos(n20_wallet.currentAccount.publicKey, psbt, utxos, network)

  const fastEstimatedSize = (psbt.txInputs.length * 51993) / 628 + ((to.length + 1) * 327) / 9 // TODO: check this formula, the result if little big than real size

  const fee = Math.floor((fastEstimatedSize * feeRate) / 1000 + 1)
  ...
  if (to.length === 1 && Number(to[0]!.amount) === totalInput) {
    ...
    psbt.addOutput({ address: to[0]!.address, value: value })
  } else {
    let totalOutput = 0
    for (const item of to) {
      psbt.addOutput({ address: item.address, value: Number(item.amount) })
    ...
    if (value < 0) throw new Error('NoFund')
    if (value > MIN_SATOSHIS) {
      psbt.addOutput({ address: change, value: value })
    }
  }
